// CareConnect app.js — v202605260152
  ...
  function renderKeypad(){
    ...
    modal.innerHTML='<div class="modal"><div style="background:#4b5563;width:100%;border-radius:32px 32px 0 0;padding:20px 16px 28px;max-height:92vh;overflow-y:auto;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><div style="font-size:13px;font-weight:900;color:#dbeafe;">'+kp.label+'</div><button id="kp-cancel" style="background:#6b7280;color:#f3f4f6;font-weight:900;font-size:12px;padding:6px 14px;border-radius:10px;border:1px solid #9ca3af;">✕ CANCEL</button></div>'+volHtml+'<div id="kp-time-box" style="background:'+(isTime?'#1e40af':'#6b7280')+';border:2px solid '+(isTime?'#60a5fa':'#9ca3af')+';border-radius:16px;padding:14px 16px;margin-bottom:16px;cursor:pointer;"><div style="font-size:10px;font-weight:900;color:'+(isTime?'#bfdbfe':'#e5e7eb')+';text-transform:uppercase;letter-spacing:0.08em;margin-bottom:4px;">Time '+((!kp.timeDigits&&!kp.ampm)?'(tap to change — using current time)':'')+'</div><div style="font-family:IBM Plex Mono,monospace;font-size:32px;font-weight:600;color:#ffffff;letter-spacing:2px;min-height:40px;display:flex;align-items:center;">'+timeDisplay+ampmHtml+'</div></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px;">'+keysHtml+'</div><button id="kp-confirm" style="width:100%;padding:16px;border-radius:16px;font-weight:900;font-size:16px;background:#2563eb;color:white;">✓ CONFIRM</button></div></div>';
  ...
  function renderInOutput(el,logs){
    ...
    el.innerHTML=
      '<div style="background:#0d3580;padding:0;height:100%;display:flex;flex-direction:column;gap:0;overflow:hidden;box-sizing:border-box;">'+

      // ── WATER ─────────────────────────────────────────────────────
      '<div style="background:#0d3580;padding:10px 10px 8px;flex:1;min-height:0;overflow:hidden;display:flex;flex-direction:column;">'+
  ...
  function renderLogs(container,todayLogs){
    ...
    container.innerHTML=
      '<div style="background:linear-gradient(160deg,#1a3a8f 0%,#2251c5 50%,#1a2d6e 100%);display:flex;flex-direction:column;height:100%;">'+
